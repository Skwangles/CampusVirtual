...
const calculatePositionFromMatrix = (
  ...
  return [
    -position.x * SCALE_TO_METRES,
    -position.y * SCALE_TO_METRES,
    position.z * SCALE_TO_METRES,
  ];
};
    ...
    while (currentKeyframeId != null) {
      ...
      for (const neighbour of neighbours.rows) {
        ...
        const node = await db.query(
          "SELECT pose FROM nodes WHERE keyframe_id = $1 LIMIT 1;",
          [neighbourId]
        );
        ...
        if (node.rows && node.rows.length > 0) {
          ...
          const dist = Math.sqrt(
            Math.pow(position[0] - currentPosition[0], 2) +
              Math.pow(position[2] - currentPosition[2], 2)
          ); // Ignore Y (index 1) because we just want flat distance
          ...
          if (dist < 5) {
            ...
            db.query(
              "INSERT INTO edges (keyframe_id1, keyframe_id0, is_direct) VALUES ($1, $2, true) ON CONFLICT DO NOTHING;",
              [degrees[0]["keyframe_id1"], degrees[1]["keyframe_id1"]]
            );
            db.query(
              "INSERT INTO edges (keyframe_id0, keyframe_id1, is_direct) VALUES ($1, $2, true) ON CONFLICT DO NOTHING;",
              [degrees[0]["keyframe_id1"], degrees[1]["keyframe_id1"]]
            );

            // Remove node and edges - including covisbilities
            db.query(
              "DELETE FROM edges WHERE (keyframe_id0 = $1 OR keyframe_id1 = $1);",
              [neighbourId]
            );
            db.query("DELETE FROM nodes WHERE keyframe_id = $1;", [
              neighbourId,
            ]);
          }
        } else {
